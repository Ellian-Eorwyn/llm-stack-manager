...
echo "[chat-backend2] Main GPU: ${CHAT_MAIN_GPU}"
echo "[chat-backend2] GPUs:    ${CUDA_VISIBLE_DEVICES} (split ${CHAT_TENSOR_SPLIT})"
echo "[chat-backend2] Device override: ${CHAT_DEVICE:-auto}"
echo "[chat-backend2] Placement: split=${CHAT_SPLIT_MODE} kv-offload=${CHAT_KV_OFFLOAD:-on} op-offload=${CHAT_OP_OFFLOAD:-on} mmproj-offload=${CHAT_MMPROJ_OFFLOAD:-on}"
echo "[chat-backend2] CPU threads:      ${CHAT_THREADS:--1} (batch=${CHAT_THREADS_BATCH:--1})"
echo "[chat-backend2] KV cache: K=${CHAT_CACHE_TYPE_K} V=${CHAT_CACHE_TYPE_V}"
echo "[chat-backend2] Reasoning format: ${CHAT_REASONING_FORMAT:-deepseek}"
echo "[chat-backend2] Fit to VRAM:      ${CHAT_FIT:-on}"
echo "[chat-backend2] Public ports: ${THINK_PORT} (think) ${NOTHINK_PORT} (nothink) via proxy"
echo "[chat-backend2] Speculative:      ${CHAT_SPEC_METHOD:-off}"
echo "[chat-backend2] Chat template:    ${CHAT_TEMPLATE_ID:-model default}"
...
if [[ -n "${CHAT_TEMPLATE_ID:-}" && "${HAS_CUSTOM_CHAT_TEMPLATE}" -eq 0 ]]; then
    if [[ ! "${CHAT_TEMPLATE_ID}" =~ ^[A-Za-z0-9._-]+$ ]]; then
        echo "[chat-backend2] Invalid CHAT_TEMPLATE_ID: ${CHAT_TEMPLATE_ID}" >&2
        exit 1
    fi
    CHAT_TEMPLATE_FILE="${STACK_DIR}/config/chat-templates/${CHAT_TEMPLATE_ID}.jinja"
    if [[ ! -f "${CHAT_TEMPLATE_FILE}" ]]; then
        echo "[chat-backend2] Chat template not found: ${CHAT_TEMPLATE_FILE}" >&2
        exit 1
    fi
...
if [[ "${SPEC_METHOD}" == "draft-model" ]]; then
    if [[ -z "${CHAT_SPEC_DRAFT_MODEL_PATH:-}" ]]; then
        echo "[chat-backend2] Speculative decoding is enabled, but CHAT_SPEC_DRAFT_MODEL_PATH is empty." >&2
        exit 1
    fi
    if [[ ! -f "${CHAT_SPEC_DRAFT_MODEL_PATH}" ]]; then
        echo "[chat-backend2] Draft model not found: ${CHAT_SPEC_DRAFT_MODEL_PATH}" >&2
        exit 1
    fi

    echo "[chat-backend2] Draft model:      ${CHAT_SPEC_DRAFT_MODEL_PATH}"
    echo "[chat-backend2] Draft GPU layers: ${CHAT_SPEC_DRAFT_N_GPU_LAYERS:-auto}"
    echo "[chat-backend2] Draft devices:    ${CHAT_SPEC_DRAFT_DEVICES:-auto}"
    echo "[chat-backend2] Draft ctx:        ${CHAT_SPEC_DRAFT_CTX_SIZE:-0}"
    echo "[chat-backend2] Draft n-max/min:  ${CHAT_SPEC_DRAFT_N_MAX:-6}/${CHAT_SPEC_DRAFT_N_MIN:-0}"
    echo "[chat-backend2] Draft p-min/split:${CHAT_SPEC_DRAFT_P_MIN:-0.75}/${CHAT_SPEC_DRAFT_P_SPLIT:-0.10}"
    ...
    if [[ ",${SPEC_METHOD}," == *,ngram-mod,* ]]; then
        echo "[chat-backend2] N-gram mod:       match=${CHAT_SPEC_NGRAM_MOD_N_MATCH:-24} min=${CHAT_SPEC_NGRAM_MOD_N_MIN:-48} max=${CHAT_SPEC_NGRAM_MOD_N_MAX:-64}"
