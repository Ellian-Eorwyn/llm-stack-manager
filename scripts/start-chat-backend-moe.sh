...
CHAT_CTX_CHECKPOINTS="${CHAT_SECONDARY_CTX_CHECKPOINTS:-${CHAT_CTX_CHECKPOINTS:-8}}"
...
CHAT_CACHE_REUSE="${CHAT_SECONDARY_CACHE_REUSE:-${CHAT_CACHE_REUSE:-256}}"
...
CHAT_REASONING_EFFORT="${CHAT_SECONDARY_REASONING_EFFORT-${CHAT_REASONING_EFFORT-}}"
...
add_swa_full_opt "[chat-backend-moe]" "${CHAT_SWA_FULL:-off}" "${CHAT_MOE_MODEL_PATH:-}"
[[ -n "${CHAT_FIT_TARGET:-}" ]] && OPTS+=(--fit-target "${CHAT_FIT_TARGET}")
add_fit_ctx_opt "[chat-backend-moe]" "${CHAT_FIT:-on}" "${CHAT_FIT_CTX:-}"
...
if [[ "${HAS_TEMPLATE_KWARGS}" -eq 0 ]]; then
    add_chat_template_kwargs_opt "[chat-backend-moe]" \
...
preflight_report "[chat-backend-moe]" chat-primary \
    "${CHAT_MOE_MODEL_PATH}" "${CHAT_MOE_MMPROJ_PATH:-}" \
    ctx_size="${CHAT_MOE_CTX_SIZE}" \
