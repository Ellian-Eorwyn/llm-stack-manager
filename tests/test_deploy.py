...
class GeneratedFilesAreIgnoredTests(unittest.TestCase):
    """What the installer writes into the tree must not make the tree dirty.

    `update.sh` refuses to run against a dirty checkout, and `deploy.py` ranks
    `dirty` above `behind` for that reason. So a file the installer generates
    into `scripts/` or `config/` and nobody ignores does not merely look untidy
    -- it disables the update path, on every host of that platform, using the
    installer's own output. That is what happened on macOS: `install.sh` writes
    a launchd wrapper and a plist per service, and all six showed up in
    `git status --porcelain`.

    Asserted through `git check-ignore` rather than by reading `.gitignore`,
    because the question is whether git ignores them, not whether a pattern
    appears somewhere.
    """

    #: One representative of each thing the installer generates in-tree.
    GENERATED = (
        "scripts/launchd-wrapper-llm-manager.sh",
        "scripts/launchd-wrapper-embed.sh",
        "config/launchd/com.llmstack.llm-manager.plist",
        "config/llm-stack.env",
        "config/models.ini",
        "config/service-expectations.json",
    )

    def test_git_ignores_everything_the_installer_generates(self):
        for path in self.GENERATED:
            with self.subTest(path):
                ...
                self.assertEqual(
                    result.returncode, 0,
                    f"{path} is generated by the installer but not ignored; "
                    f"it would make every install of that platform dirty, and "
                    f"update.sh refuses to run against a dirty tree")

    def test_the_tracked_launchers_are_still_tracked(self):
        """The guard on the guard: an over-broad pattern that swallowed the real
        launchers would pass the test above and delete the stack."""
        for path in ("scripts/start-llm-a.sh", "scripts/start-backend.sh",
                     "scripts/cross-platform.sh"):
            with self.subTest(path):
                result = subprocess.run(
                    ["git", "check-ignore", "-q", path],
                    cwd=ROOT, capture_output=True)
                self.assertNotEqual(result.returncode, 0, f"{path} is ignored")
